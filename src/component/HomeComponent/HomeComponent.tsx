...
export function HomeComponent() {
  ...
  return (
    <div className="container max-w-full flex-grow bg-white/[.85] dark:bg-slate-800 text-slate-900 dark:text-white overflow-auto dark:[color-scheme:dark]">
        <SearchBarComponent searchString={searchString} searchHandler={searchHandler} disable={isLoading}></SearchBarComponent>
        {error ? <ErrorPopUpComponent error={error} handleCloseError={handleCloseError}></ErrorPopUpComponent> : null}
        {isLoading ?
        ...
        <div className="grid grid-cols-4 gap-4 place-items-center">
            {!isLoading ? currentTableData && currentTableData.map(item => {
            return (
              <GitHubRepoCardComponent name={item.name} startgazerCount={item.stargazers_count} lastUpdate={item.updated_at} description={item.description} imgURL={item.owner.avatar_url} repoUrl={item.html_url}></GitHubRepoCardComponent>
            );
          })
          : null
        }
        ...
        {nbPage > 0 && !isLoading && <div className="flex flex-col items-center py-2">
          <span className="text-sm text-gray-700 dark:text-gray-400">
              Showing <span className="font-semibold text-gray-900 dark:text-white"> {1 + (currentPage * pageSize)} </span> 
               to 
              <span className="font-semibold text-gray-900 dark:text-white"> {apiResponse && apiResponse.total_count < ((currentPage+1) * pageSize) ? apiResponse.total_count : ((currentPage+1) * pageSize)} </span> 
               of 
              <span className="font-semibold text-gray-900 dark:text-white"> {apiResponse && apiResponse.total_count} </span> 
              Entries
          </span>
          <div className="inline-flex mt-2 xs:mt-0">
              <button onClick={() => {setCurrentPage(currentPage-1); setApiPage(Math.floor((currentPage-1) / pagePerRefresh)+1)}} className="py-2 px-4 text-sm font-medium text-white bg-gray-800 rounded-l enabled:hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 enabled:dark:hover:bg-gray-700 enabled:dark:hover:text-white disabled:opacity-50" disabled={currentPage<1}>
                  Prev
              </button>
              <button onClick={() => {setCurrentPage(currentPage+1); setApiPage(Math.floor((currentPage+1) / pagePerRefresh)+1)} } className="py-2 px-4 text-sm font-medium text-white bg-gray-800 rounded-r border-0 border-l border-gray-700 enabled:hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 enabled:dark:hover:bg-gray-700 enabled:dark:hover:text-white disabled:opacity-50" disabled={nbPage<=currentPage}>
                  Next
              </button>
          </div>
        </div>}
